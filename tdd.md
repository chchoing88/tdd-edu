...
객체에 수행해야 할 모든 것들을 담아 뒀다가 필요할 때 실행 시킨다.(run)

### 값 객체

널리 공유해야 하지만 동일성은 중요하지 않을 때 값 객체로 설계한다.
객체가 생성된 이후 그 값이 절대로 변하지 않게 하여 별칭 문제가 발생하지 않게 한다.

그리고 이 객체에 대해 수행되는 연산은 언제나 새로운 객체를 반환하게 만들다.

객체의 복사본을 알려주거나 옵저버 패턴을 이용해서 원할때 객체의 상태가 변하면 통지를 받는 방법이다.
또는 이 객체가 변하지 않을 것임을 안다면 이 객체에 대한 참조를 원하는 곳 어디로든 넘겨줄 수 있다.

모든 오퍼레이션은 새로운 객체를 반환해야 한다.

교차하고 합해지는 기하학적 모양들, 숫자와 함께 그 단위가 따라다니는 단위값, 기호대수학등 대수학 같아 보이는 상황이라면 언제나 값 객체를 사용하려는 경향이 있다.
